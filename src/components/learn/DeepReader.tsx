...
import type { Phase2DisplayMode } from "@/lib/db/experiences";
...
// SECTION DISPATCHER
// ──────────────────────────────────────────────────────────────
function SectionBlock({ section, inView }: { section: DeepSection; inView: boolean }) {
...
function AnimatedSection({ section, index }: { section: DeepSection; index: number }) {
...
export function DeepReaderLoadingScreen() {
  return (
...
export function DeepReaderErrorScreen({ message }: { message: string }) {
  return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <p style={{ color: "#fca5a5", fontSize: 15 }}>{message}</p>
    </div>
  );
}
