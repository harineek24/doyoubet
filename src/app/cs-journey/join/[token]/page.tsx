      ...
      <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        {!user ? (
          <div style={{ textAlign: "center" }}>
            ...
            <button onClick={() => router.push(`/login?next=/cs-journey/join/${token}`)} style={{ fontFamily: SERIF, fontStyle: "italic", fontWeight: 700, fontSize: "0.95rem", color: "#fffdf8", background: "linear-gradient(135deg, #f59e0b, #d97706)", border: "none", borderRadius: 50, padding: "12px 32px", cursor: "pointer", boxShadow: "0 6px 20px rgba(245,158,11,0.35)" }}>
              Sign in to continue
            </button>
          </div>
        ) : added ? (
